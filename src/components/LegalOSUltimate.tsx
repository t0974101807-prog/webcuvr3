...
export default function LegalOSUltimate() {
  ...
  return (
    <div className="bg-[#0b1b2d] border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6" id="legal_os_ultimate_module">
      
      {/* 1. Cockpit Header */}
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center space-x-3">
          <div className="bg-[#e2b13c]/15 p-2 rounded-xl border border-[#e2b13c]/30 text-[#e2b13c]">
            <ShieldCheck size={20} className="animate-pulse" />
          </div>
          <div>
            <h3 className="text-xs font-black text-white tracking-widest uppercase">LEGAL OS HARDENED COCKPIT</h3>
            <p className="text-[10px] text-slate-400 font-bold">Mô phỏng phòng thủ an ninh thông tin & Tải cao văn phòng Ánh Dương Law</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-3.5">
          <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider block">Trạng thái hệ thống</span>
          <span className={`text-sm font-black mt-1 block ${serverStats.status === "HEALTHY" ? "text-emerald-400" : "text-red-500 animate-pulse"}`}>{serverStats.status}</span>
        </div>
        <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-3.5">
          <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider block">Sử dụng CPU</span>
          <span className="text-sm font-black text-white mt-1 block">{serverStats.cpu}</span>
        </div>
        <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-3.5">
          <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider block">Sử dụng RAM</span>
          <span className="text-sm font-black text-white mt-1 block">{serverStats.ram}</span>
        </div>
        <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-3.5">
          <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider block">Băng thông Server</span>
          <span className="text-sm font-black text-white mt-1 block">{serverStats.bandwidth}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-950/40 border border-slate-900 rounded-2xl p-5 space-y-4">
          <div className="flex justify-between items-center border-b border-slate-900 pb-2.5">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center space-x-1.5">
              <Database size={12} className="text-[#e2b13c]" />
              <span>Bảng cơ sở dữ liệu (Không dùng UUID)</span>
            </h4>
            <button 
              onClick={handleCreateCase}
              className="py-1 px-2.5 bg-[#e2b13c] hover:bg-amber-500 text-[#070e17] text-[9px] font-black rounded-md flex items-center space-x-1 transition"
            >
              <Plus size={10} />
          ...
          <div className="space-y-2.5 max-h-[180px] overflow-y-auto pr-1">
            {cases.map((c, i) => {
              ...
              return (
                <div key={i} className="bg-[#050c14] border border-slate-900 p-3 rounded-xl space-y-2 text-[11px] font-bold">
                  <div className="flex justify-between items-center">
                    <span className="text-red-400 font-black text-[9px] flex items-center space-x-1">
                      <Lock size={9} />
                      <span>ID thật nội bộ (Ẩn): {c.id}</span>
                    </span>
                    <span className="text-emerald-400 text-[9px]">● Đã băm cơ số toán học</span>
                  </div>
                  <div className="space-y-0.5 bg-slate-950/60 p-2 rounded-lg border border-slate-900 font-mono text-[9px] text-slate-400">
                    <p className="text-[#e2b13c] font-black">shortId (Lộ ra Client API): {shortId}</p>
                    <p className="text-[8px]">URL truy cập: sunshinelaw.com/cases/{shortId}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="bg-slate-950/40 border border-slate-900 rounded-2xl p-5 space-y-4">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-900 pb-2.5 flex items-center space-x-1.5">
            <Cpu size={12} className="text-[#e2b13c]" />
            <span>Mô phỏng 1000 người dùng tải file đồng thời</span>
          </h4>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="bg-[#050c14] border border-red-950/20 p-3 rounded-xl space-y-2.5">
              <h5 className="text-[10px] font-black text-red-400">Làm trung gian (Intern)</h5>
              <p className="text-[9px] text-slate-500 leading-relaxed font-bold">Mọi byte dữ liệu dồn về RAM của máy chủ trước khi chuyển tiếp lên mây.</p>
              <button 
                onClick={handleTriggerInternSimulation}
                className="w-full py-1.5 bg-red-950/30 hover:bg-red-900/30 border border-red-900/50 text-red-400 text-[9px] font-black rounded-md transition"
              >
                Mô phỏng treo máy (Proxy)
              </button>
            </div>

            <div className="bg-[#050c14] border border-[#e2b13c]/10 p-3 rounded-xl space-y-2.5">
              <h5 className="text-[10px] font-black text-[#e2b13c]">Giải pháp Senior (Presigned URL)</h5>
              <p className="text-[9px] text-slate-500 leading-relaxed font-bold">Server chỉ cấp chữ ký số (2ms). File bytes truyền trực tiếp từ Client lên GCS.</p>
              <button 
                onClick={handleTriggerSeniorSimulation}
                className="w-full py-1.5 bg-[#e2b13c] hover:bg-amber-500 text-[#070e17] text-[9px] font-black rounded-md transition"
              >
          ...
          {/* SIMULATOR LOGS */}
          <div className="h-[90px] overflow-y-auto bg-slate-950/90 p-3 rounded-xl border border-slate-900 font-mono text-[9px] text-slate-400 space-y-1">
            {simLogs.length === 0 ? (
