...
const startMarker = '{/* Overview Cards */}';
const endMarker = '{/* Line Chart & Categories */}';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-5 rounded-xl shadow-lg border border-indigo-400/30 flex flex-col justify-between hover:shadow-xl transition-all cursor-pointer relative overflow-hidden group" onClick={() => { setActiveTab('records'); setTimeout(() => window.dispatchEvent(new CustomEvent('set-record-tab', {detail: 'Đang xử lý'})), 50); }}>
          ...
          <div className="flex justify-between items-start mb-3 relative z-10">
            ...
            <div className="p-1.5 bg-white/20 rounded-lg backdrop-blur-sm text-white shadow-sm">
              <Clock size={16} />
            </div>
          </div>
          <div className="flex items-baseline gap-2 mt-auto relative z-10">
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-5 rounded-xl shadow-lg border border-amber-400/30 flex flex-col justify-between hover:shadow-xl transition-all cursor-pointer relative overflow-hidden group" onClick={() => setActiveTab('events')}>
          ...
          <div className="flex justify-between items-start mb-3 relative z-10">
            ...
            <div className="p-1.5 bg-white/20 rounded-lg text-white backdrop-blur-sm shadow-sm">
              <Gavel size={16} />
            </div>
          </div>
          <div className="flex items-baseline gap-2 mt-auto relative z-10">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all relative overflow-hidden group">
        ...
        {/* HOÀN THÀNH */}
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all cursor-pointer relative overflow-hidden group" onClick={() => { setActiveTab('records'); setTimeout(() => window.dispatchEvent(new CustomEvent('set-record-tab', {detail: 'hoàn thành'})), 50); }}>
          <div className="flex justify-between items-start mb-3">
        ...
        {/* LỊCH HẸN SẮP TỚI */}
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all cursor-pointer relative overflow-hidden group" onClick={() => setActiveTab('events')}>
          <div className="flex justify-between items-start mb-3">
...
\n      `;
  ...
  content = content.substring(0, startIndex) + newKpiUI + content.substring(endIndex);
  fs.writeFileSync('src/components/ERP.tsx', content);
  console.log('Fixed overlapping JSX format!');
} else {
